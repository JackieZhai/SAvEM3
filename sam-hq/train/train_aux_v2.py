...
from utils.dataloader_ng import create_dataloaders, RandomHFlip, RandomVFlip
from utils.loss_mask import loss_masks
from utils.location import data_location_list, data_range_list, data_resolution_list
...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type, num_auxiliary_channels=1):
        ...
        self.aux_dim = num_auxiliary_channels

        self.hf_token = nn.Embedding(1, transformer_dim)
        self.hfa_token = nn.Embedding(self.aux_dim, transformer_dim)
        self.hf_mlp = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
        self.hfa_mlp = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
        self.num_mask_tokens = self.num_mask_tokens + 1 + self.aux_dim
    ...
    def forward(
        ...
        masks_hq = masks[:,slice(self.num_mask_tokens-self.aux_dim-1, self.num_mask_tokens-self.aux_dim), :, :]
        masks_hqa = masks[:,slice(self.num_mask_tokens-self.aux_dim, self.num_mask_tokens), :, :]
        
        if hq_token_only:
            return masks_hq, masks_hqa
        else:
            return masks_sam, masks_hq, masks_hqa
    ...
    def predict_masks(
        ...
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight, 
                                   self.hf_token.weight, self.hfa_token.weight], dim=0)
        ...
        for i in range(self.num_mask_tokens):
            if i < 4:
                ...
                hyper_in_list.append(self.hfa_mlp(mask_tokens_out[:, i, :]))
...
def main(net, args):
    ...
    if not args.eval:
        ...
        train_dataloaders, _ = create_dataloaders(args.root, args.root_cache,
                                                  data_name_list, data_location_list, data_range_list, data_resolution_list,
                                                  my_transforms=[RandomHFlip(), RandomVFlip()],
                                                  batch_size=1, eval=False,
                                                  fast=args.fast, aux_dim=args.aux_dim)
        print(len(train_dataloaders), " train dataloaders created")
        for i in range(len(train_dataloaders)):
            print("dataloader {:d}:".format(i+1), len(train_dataloaders[i]), "steps created")

    print("--- create valid dataloader ---")
    data_name_list = [
        ['snemi'],
    ]
    valid_dataloaders, _ = create_dataloaders(args.root, args.root_cache,
                                              data_name_list, data_location_list, data_range_list, data_resolution_list,
                                              my_transforms=[],
    ...
    for i in range(len(valid_dataloaders)):
        print("dataloader {:d}:".format(i+1), len(valid_dataloaders[i]), "steps created")
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start,epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,1000):
            labels, labels_list, auxlabels, embeds, intembeds = data['label'], data['label_list'], data['auxiliary_label'], data['embedding'], data['interm_embedding']
            if torch.cuda.is_available():
                labels = labels.cuda()
                auxlabels = auxlabels.cuda()
                embeds = embeds.cuda()
                intembeds = intembeds.cuda()

            # input prompt
            labels_box = torch.Tensor([0, 0, labels.shape[-1], labels.shape[-2]], device=labels.device).view(1, 4).expand(labels.shape[0], 4)
            labels_points = misc.kimimaro_masks_sample_points(labels[:,0,:,:], labels_list)
            
            batched_input = []
            for b_i in range(labels.shape[0]):
                dict_input = dict()
                dict_input['embedding'] = embeds[b_i:b_i+1]
            ...
            for c_i in range(auxlabels.shape[1]):
                loss_mask_aux, loss_dice_aux = loss_masks(mask_hqa[:, c_i, :, :], auxlabels[:, c_i, :, :]/255.0, len(mask_hqa))
                loss += loss_mask_aux + loss_dice_aux
                if c_i == 0:
                    loss_dict["loss_mask_mem"] = loss_mask_aux
                    loss_dict["loss_dice_mem"] = loss_dice_aux
                elif c_i == 1:
                    loss_dict["loss_mask_dst"] = loss_mask_aux
                    loss_dict["loss_dice_dst"] = loss_dice_aux
                else:
...
def evaluate(args, net, sam, valid_dataloaders, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader,1000):
            ...
            masks_sam, masks_hq, masks_hqa = net(
