...
def write_vol(filename, data, dataset_names=None):
    """Write h5/tif volume. `data` and `dataset_names` can be single values or lists."""
    if isinstance(data, (list, tuple)):
        data_list = list(data)
        names = list(dataset_names) if dataset_names is not None else ['main'] * len(data_list)
    else:
        data_list = [data]
        names = [dataset_names or 'main']
    if '.h5' in filename:
        with h5py.File(filename, 'w') as f:
            for name, arr in zip(names, data_list):
                f.create_dataset(name, data=arr)
    elif '.tif' in filename or '.tiff' in filename:
        from imageio import volwrite
        volwrite(filename, np.asarray(data_list[0]))
    else:
        raise ValueError('cannot recognize output file type: ' + filename)
