...
def show_affs(iters, inputs, pred, target, cache_path, model_type='mala', suffix=None):
    pred = pred[0]; inputs = inputs[0]; target = target[0]
    ...
    if suffix is not None:
        Image.fromarray(im_cat).save(os.path.join(cache_path, '%06d_%s.png' % (iters, suffix)))
    else:
        Image.fromarray(im_cat).save(os.path.join(cache_path, '%06d.png' % iters))
...
def show_affs3(iters, inputs, pred, target, cache_path, model_type='mala'):
    pred = pred[0].data.cpu().numpy()
    inputs = inputs[0].data.cpu().numpy()
    # target = target[0].data.cpu().numpy()
    inputs = np.squeeze(inputs)
...
def show_affs_whole(iters, out_affs, gt_affs, cache_path):
    z_mid = out_affs.shape[1] // 2
    out_affs = out_affs[:, z_mid, ...]
    gt_affs = gt_affs[:, z_mid, ...]
...
def show_affs_feats_embeds(iters, out_affs, out_feats, out_embeds, gt_affs, cache_path, z_mid=None):
    if z_mid is None:
        z_mid = out_affs.shape[1] // 2
    out_affs = out_affs[:, z_mid, ...]
    gt_affs = gt_affs[:, z_mid, ...]
    ...
    im_cat = np.concatenate([out_affs, gt_affs], axis=1)
    feats_list = []
    for c in range(0, out_feats.shape[0], out_feats.shape[0]//8):
        out_feat = out_feats[c:c+1, z_mid, ...]
        out_feat = (out_feat - np.min(out_feat)) / (np.max(out_feat) - np.min(out_feat)) * 255
        out_feat = out_feat.astype(np.uint8)
        out_feat = np.repeat(out_feat, 3, axis=0)
        out_feat = np.transpose(out_feat, (1,2,0))
        feats_list.append(out_feat)
    im_feat_cat = np.concatenate(feats_list, axis=1)
    embeds_list = []
    for c in range(0, out_embeds.shape[0], out_embeds.shape[0]//32):
        out_embed = out_embeds[c:c+1, z_mid, ...]
        c_min = np.percentile(out_embeds[c, ...], 5)
        c_max = np.percentile(out_embeds[c, ...], 95)
        out_embed = (out_embed - c_min) / (c_max - c_min) * 255
        out_embed = out_embed.astype(np.uint8)
        out_embed = np.repeat(out_embed, 3, axis=0)
        out_embed = np.transpose(out_embed, (1,2,0))
        embeds_list.append(out_embed)
    im_embed_cat = np.concatenate(embeds_list, axis=1)
    im_cat = np.concatenate([im_cat, im_feat_cat, im_embed_cat], axis=0)
    Image.fromarray(im_cat).save(os.path.join(cache_path, '{:06d}_z{:04d}.png'.format(iters, z_mid)))
