...
class UNetS_PNI(nn.Module):  # deployed PNI model 
    def __init__(self, in_planes=1, 
                    out_planes=3, 
                    filters=[32, 64, 128, 256],    # [16, 32, 64, 128], [32, 64, 128, 256]
                    upsample_mode='transposeS',  # transposeS, bilinear
                    decode_ratio=1, 
                    merge_mode='cat', 
                    pad_mode='zero', 
        ...
        self.center = resBlock_pni(filters2[3], filters2[4], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)

        # upsample stream
        self.up0 = upsampleBlock(filters2[4], filters2[3], (1,2,2), upsample_mode, init_mode=init_mode)
        if self.merge_mode == 'add':
            self.cat0 = conv3dBlock([0], [filters2[3]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv4 = resBlock_pni(filters2[3], filters2[3], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        else:
            self.cat0 = conv3dBlock([0], [filters2[3]*2], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv4 = resBlock_pni(filters2[3]*2, filters2[3], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)

        self.up1 = upsampleBlock(filters2[3], filters2[2], (1,2,2), upsample_mode, init_mode=init_mode)
        if self.merge_mode == 'add':
            self.cat1 = conv3dBlock([0], [filters2[2]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv5 = resBlock_pni(filters2[2], filters2[2], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        else:
            self.cat1 = conv3dBlock([0], [filters2[2]*2], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv5 = resBlock_pni(filters2[2]*2, filters2[2], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)

        self.up2 = upsampleBlock(filters2[2], filters2[1], (1,2,2), upsample_mode, init_mode=init_mode)
        if self.merge_mode == 'add':
            self.cat2 = conv3dBlock([0], [filters2[1]], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv6 = resBlock_pni(filters2[1], filters2[1], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
        else:
            self.cat2 = conv3dBlock([0], [filters2[1]*2], bn_mode=[bn_mode], relu_mode=[relu_mode], bn_momentum=bn_momentum)
            self.conv6 = resBlock_pni(filters2[1]*2, filters2[1], pad_mode, bn_mode, relu_mode, init_mode, bn_momentum)
    ...
    def forward(self, x):
        ...
        up0 = self.up0(center)
        if self.merge_mode == 'add':
            cat0 = self.cat0(up0 + conv2)
        else:
            cat0 = self.cat0(torch.cat([up0, conv2], dim=1))
        conv4 = self.conv4(cat0)

        up1 = self.up1(conv4)
        if self.merge_mode == 'add':
            cat1 = self.cat1(up1 + conv1)
        else:
            cat1 = self.cat1(torch.cat([up1, conv1], dim=1))
        conv5 = self.conv5(cat1)

        up2 = self.up2(conv5)
        if self.merge_mode == 'add':
            cat2 = self.cat2(up2 + conv0)
        else:
            cat2 = self.cat2(torch.cat([up2, conv0], dim=1))
        conv6 = self.conv6(cat2)

        embed_out = self.embed_out(conv6)
        out = self.out_put(embed_out)
        ...
        # if self.show_feature:
        #     down_features = [conv0, conv1, conv2]
        #     center_features = [center]
        #     up_features = [conv4, conv5, conv6]
        #     return down_features, center_features, up_features, out
        # else:
        #     return out
        if self.show_feature:
...
class UNetST_PNI(nn.Module):  # deployed PNI model 
    def __init__(self, in_planes=1, 
                    out_planes=3, 
                    filters=[32, 64, 128, 256],    # [16, 32, 64, 128], [32, 64, 128, 256]
                    upsample_mode='transposeS',  # transposeS, bilinear
                    ...
                    show_feature=False):
        # filter_ratio: #filter_decode/#filter_encode
        super(UNetST_PNI, self).__init__()
        filters2 = filters[:1] + filters
        self.merge_mode = merge_mode
        self.do_embed = do_embed
        self.depth = len(filters2) - 2
...
if __name__ == "__main__":
    import numpy as np
    # input_size = (1, 18, 160, 160)
    # input_size = (1, 20, 120, 120)
    input_size = (1, 30, 120, 120)
    input = np.random.random((1,)+input_size).astype(np.float32)
    x = torch.tensor(input)

    # model = UNet_PNI(filters=[28, 36, 48, 64, 80], upsample_mode='bilinear', merge_mode='add')  # .to('cuda:0')
    # model = UNet_PNI(filters=[4, 18, 32, 144, 256], upsample_mode='transposeS', merge_mode='add')
    # model = UNetS_PNI(out_planes=1, filters=[16, 32, 144, 256], upsample_mode='transposeS', merge_mode='add')
    model = UNetS_PNI(out_planes=1, filters=[12, 32, 90, 256], upsample_mode='transposeS', merge_mode='add')
