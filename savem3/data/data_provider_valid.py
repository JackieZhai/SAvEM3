...
class Provider_valid(Dataset):
    def __init__(self, cfg, valid_data=None, num_z=18, test=False, test_split=None):
        ...
        elif valid_dataset_name in ['ac4', 'ac4-triplet', 'ac4-triplet-dst', 'ac4-triplet-flow']:
            self.sub_path = 'AC3-AC4'
        ...
        if cfg.MODEL.model_type == 'mala':
            ...
            if self.dataset[0].shape[0] == 200:
                ...
                self.stride = [10, 80, 80]
                # self.valid_padding = [4, padding_xy, padding_xy]
                self.valid_padding = [5, padding_xy, padding_xy]  # for input_size z=30
                self.num_zyx = [2, num_xy, num_xy]
            else:
    ...
    def reset_output(self, default_c=None):
        ...
        if self.model_type == 'superhuman':
            ...
            if hasattr(self, 'out_feats'):
                del self.out_feats
            if hasattr(self, 'weight_map_x4'):
                del self.weight_map_x4
                del self.weight_vol_x4
            if hasattr(self, 'out_embeds'):
                del self.out_embeds
            if hasattr(self, 'weight_map_x16'):
                del self.weight_map_x16
                del self.weight_vol_x16
        else:
    ...
    def get_weight(self, sigma=0.2, mu=0.0, out_size=None):
        if out_size is None:
            out_size = self.out_size
        if self.num_z < 18:
            zz, yy, xx = np.meshgrid(np.linspace(-1, 1, 18, dtype=np.float32),
                                    np.linspace(-1, 1, out_size[1], dtype=np.float32),
                                    np.linspace(-1, 1, out_size[2], dtype=np.float32), indexing='ij')
        else:
            zz, yy, xx = np.meshgrid(np.linspace(-1, 1, out_size[0], dtype=np.float32),
                                    np.linspace(-1, 1, out_size[1], dtype=np.float32),
                                    np.linspace(-1, 1, out_size[2], dtype=np.float32), indexing='ij')
    ...
    def add_vol(self, affs_vol):
        ...
        if self.model_shallow:
            affs_tmp = np.zeros((affs_vol.shape[0], affs_vol.shape[1], 
                                 affs_vol.shape[2]*2, affs_vol.shape[3]*2), dtype=affs_vol.dtype)
            for c in range(affs_vol.shape[0]):
                for z in range(affs_vol.shape[1]):
                    affs_tmp[c, z] = cv2.resize(affs_vol[c, z], (0, 0), fx=2, fy=2, interpolation=cv2.INTER_LINEAR)
            affs_vol = affs_tmp
    ...
    def add_vol_all(self, affs_vol, feat_vol, embed_vol):
        fromz, fromy, fromx = self.pos
        if self.num_z < 18:
            raise NotImplementedError
        ...
        if self.model_type == 'superhuman':
            ...
            self.weight_map[:, fromz:fromz+self.out_size[0], \
                            fromx:fromx+self.out_size[1], \
                            fromy:fromy+self.out_size[2]] += self.weight_vol
            if not hasattr(self, 'out_feats'):
                self.out_feats = np.zeros(tuple(
                    [feat_vol.shape[0]]+
                    [self.raw_data_shape[0], self.raw_data_shape[1]//4, self.raw_data_shape[2]//4]
                    ), dtype=np.float32)
            if not hasattr(self, 'weight_map_x4'):
                self.weight_map_x4 = np.zeros(tuple(
                    [1]+[self.raw_data_shape[0], self.raw_data_shape[1]//4, self.raw_data_shape[2]//4]
                    ), dtype=np.float32)
                self.weight_vol_x4 = self.get_weight(out_size=[self.out_size[0], self.out_size[1]//4, self.out_size[2]//4])
            if not hasattr(self, 'out_embeds'):
                self.out_embeds = np.zeros(tuple(
                    [embed_vol.shape[0]]+
                    [self.raw_data_shape[0], self.raw_data_shape[1]//16, self.raw_data_shape[2]//16]
                    ), dtype=np.float32)
            if not hasattr(self, 'weight_map_x16'):
                self.weight_map_x16 = np.zeros(tuple(
                    [1]+[self.raw_data_shape[0], self.raw_data_shape[1]//16, self.raw_data_shape[2]//16]
                    ), dtype=np.float32)
                self.weight_vol_x16 = self.get_weight(out_size=[self.out_size[0], self.out_size[1]//16, self.out_size[2]//16])
            self.out_feats[:, fromz:fromz+self.out_size[0], \
                            fromx//4:fromx//4+self.out_size[1]//4, \
                            fromy//4:fromy//4+self.out_size[2]//4] += feat_vol * self.weight_vol_x4
            self.out_embeds[:, fromz:fromz+self.out_size[0], \
                            fromx//16:fromx//16+self.out_size[1]//16, \
                            fromy//16:fromy//16+self.out_size[2]//16] += embed_vol * self.weight_vol_x16
            self.weight_map_x4[:, fromz:fromz+self.out_size[0], \
                            fromx//4:fromx//4+self.out_size[1]//4, \
                            fromy//4:fromy//4+self.out_size[2]//4] += self.weight_vol_x4
            self.weight_map_x16[:, fromz:fromz+self.out_size[0], \
                            fromx//16:fromx//16+self.out_size[1]//16, \
                            fromy//16:fromy//16+self.out_size[2]//16] += self.weight_vol_x16
        else:
            self.out_affs[:, fromz:fromz+self.out_size[0], \
                             fromx:fromx+self.out_size[1], \
                             fromy:fromy+self.out_size[2]] = affs_vol
    ...
    def get_results_all(self):
        if self.model_type == 'superhuman':
            self.out_affs = self.out_affs / self.weight_map
            self.out_feats = self.out_feats / self.weight_map_x4
            self.out_embeds = self.out_embeds / self.weight_map_x16
            if self.valid_padding[0] == 0:
                self.out_affs = self.out_affs[:, :, \
                                                self.valid_padding[1]:-self.valid_padding[1], \
                                                self.valid_padding[2]:-self.valid_padding[2]]
                self.out_feats = self.out_feats[:, :, \
                                                self.valid_padding[1]//4:-self.valid_padding[1]//4, \
                                                self.valid_padding[2]//4:-self.valid_padding[2]//4]
                self.out_embeds = self.out_embeds[:, :, \
                                                self.valid_padding[1]//16:-self.valid_padding[1]//16, \
                                                self.valid_padding[2]//16:-self.valid_padding[2]//16]
            else:
                self.out_affs = self.out_affs[:, self.valid_padding[0]:-self.valid_padding[0], \
                                                self.valid_padding[1]:-self.valid_padding[1], \
                                                self.valid_padding[2]:-self.valid_padding[2]]
                self.out_feats = self.out_feats[:, self.valid_padding[0]:-self.valid_padding[0], \
                                                self.valid_padding[1]//4:-self.valid_padding[1]//4, \
                                                self.valid_padding[2]//4:-self.valid_padding[2]//4]
                self.out_embeds = self.out_embeds[:, self.valid_padding[0]:-self.valid_padding[0], \
                                                self.valid_padding[1]//16:-self.valid_padding[1]//16, \
                                                self.valid_padding[2]//16:-self.valid_padding[2]//16]
        return self.out_affs, self.out_feats, self.out_embeds
