...
from loss.loss import MSELoss, BCELoss, L1Loss
...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        if 'triplet' in cfg.MODEL.model_type:
            ...
            if cfg.MODEL.model_type in ['superhuman_devo-t3_triplet-MSE_shallow']:
                loss_feat = criterion_feat(pred_feat[:, :32, :, :, :], target_feat, weightmap_x4)
            else:
                loss_feat = criterion_feat(pred_feat, target_feat, weightmap_x4)
            loss_embed = criterion_embed(pred_embed, target_embed, weightmap_x16)
            loss_all = loss + cfg.TRAIN.loss_lambda_feat * loss_feat + cfg.TRAIN.loss_lambda_embed * loss_embed
            loss_all.backward()
        else:
            pred = model(inputs)
            loss = criterion(pred, target, weightmap)
        ...
        if iters % cfg.TRAIN.display_freq == 0 or iters == 1:
            ...
            if 'triplet' in cfg.MODEL.model_type:
                if iters == 1:
                    logging.info('step %d, loss = %.6f, loss_feat = %.6f, loss_embed = %.6f (wt: *1, lr: %.8f, et: %.2f sec, rd: %.2f min)'
                                % (iters, sum_loss * 1, sum_loss_feat * 1, sum_loss_embed * 1, current_lr, sum_time,
                                (cfg.TRAIN.total_iters - iters) / cfg.TRAIN.display_freq * np.mean(np.asarray(rcd_time)) / 60))
                    writer.add_scalar('loss', sum_loss * 1, iters)
                    writer.add_scalar('loss_feat', sum_loss_feat * 1, iters)
                    writer.add_scalar('loss_embed', sum_loss_embed * 1, iters)
                else:
                    logging.info('step %d, loss = %.6f, loss_feat = %.6f, loss_embed = %.6f (wt: *1, lr: %.8f, et: %.2f sec, rd: %.2f min)'
                                % (iters, sum_loss / cfg.TRAIN.display_freq * 1, sum_loss_feat / cfg.TRAIN.display_freq * 1, 
                                sum_loss_embed / cfg.TRAIN.display_freq * 1, current_lr, sum_time,
                                (cfg.TRAIN.total_iters - iters) / cfg.TRAIN.display_freq * np.mean(np.asarray(rcd_time)) / 60))
                    writer.add_scalar('loss', sum_loss / cfg.TRAIN.display_freq * 1, iters)
                    writer.add_scalar('loss_feat', sum_loss_feat / cfg.TRAIN.display_freq * 1, iters)
                    writer.add_scalar('loss_embed', sum_loss_embed / cfg.TRAIN.display_freq * 1, iters)
            else:
                if iters == 1:
            ...
            if 'triplet' in cfg.MODEL.model_type:
                f_loss_txt.write('step = ' + str(iters) + ', loss = ' + str(sum_loss / cfg.TRAIN.display_freq * 1) + 
                                 ', loss_feat = ' + str(sum_loss_feat / cfg.TRAIN.display_freq * 1) + 
                                 ', loss_embed = ' + str(sum_loss_embed / cfg.TRAIN.display_freq * 1))
            else:
                f_loss_txt.write('step = ' + str(iters) + ', loss = ' + str(sum_loss / cfg.TRAIN.display_freq * 1))
            f_loss_txt.write('\n')
            f_loss_txt.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='mem3_3ds_t1', help='path to config file')
...
CUDA_VISIBLE_DEVICES=0 /mnt/WGCJ/miniconda4/envs/micro-sam/bin/python main_devoem_sparse_membrane_triplet.py -c mem3c_3ds_t3

CUDA_VISIBLE_DEVICES=1 /mnt/WGCJ/miniconda4/envs/micro-sam/bin/python main_devoem_sparse_membrane_triplet.py -c mem3b_3ds_t3

CUDA_VISIBLE_DEVICES=2 /mnt/WGCJ/miniconda4/envs/micro-sam/bin/python main_devoem_sparse_membrane_triplet.py -c mem3e_3ds_t3

CUDA_VISIBLE_DEVICES=3 /mnt/WGCJ/miniconda4/envs/micro-sam/bin/python main_devoem_sparse_membrane_triplet.py -c mem3d_3ds_t3

CUDA_VISIBLE_DEVICES=1 /mnt/WGCJ/miniconda4/envs/micro-sam/bin/python main_devoem_sparse_membrane_triplet.py -c mem3c_3ds_t3_b16
'''