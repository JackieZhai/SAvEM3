...
class Rescale(DataAugment):
    ...
    def apply_rescale_neck(self, image, label, feature, embedding, sf_x, sf_y, random_state):
        # apply image, mask, feature and embedding at the same time
        transformed_image = image.copy()
        transformed_label = label.copy()
        ...
        if y_length <= image.shape[1]:
            ...
            transformed_image = transformed_image[:, y0:y1, :]
            transformed_label = transformed_label[:, y0:y1, :]
            transformed_feature = transformed_feature[:, :, y0//4:y1//4, :]
            transformed_embedding = transformed_embedding[:, :, y0//16:y1//16, :]
        else:
            y0 = int(np.floor((y_length - image.shape[1]) / 2))
            y1 = int(np.ceil((y_length - image.shape[1]) / 2))
            transformed_image = np.pad(transformed_image, ((0, 0),(y0, y1),(0, 0)), mode='constant')
            transformed_label = np.pad(transformed_label, ((0, 0),(y0, y1),(0, 0)), mode='constant')
            transformed_feature = np.pad(transformed_feature, ((0, 0),(0, 0),(y0//4, y1//4),(0, 0)), mode='constant')
            transformed_embedding = np.pad(transformed_embedding, ((0, 0),(0, 0),(y0//16, y1//16),(0, 0)), mode='constant')
        ...
        if x_length <= image.shape[2]:
            ...
            transformed_image = transformed_image[:, :, x0:x1]
            transformed_label = transformed_label[:, :, x0:x1]
            transformed_feature = transformed_feature[:, :, :, x0//4:x1//4]
            transformed_embedding = transformed_embedding[:, :, :, x0//16:x1//16]
        else:
            x0 = int(np.floor((x_length - image.shape[2]) / 2))
            x1 = int(np.ceil((x_length - image.shape[2]) / 2))
            transformed_image = np.pad(transformed_image, ((0, 0),(0, 0),(x0, x1)), mode='constant')
            transformed_label = np.pad(transformed_label, ((0, 0),(0, 0),(x0, x1)), mode='constant')
            transformed_feature = np.pad(transformed_feature, ((0, 0),(0, 0),(0, 0),(x0//4, x1//4)), mode='constant')
            transformed_embedding = np.pad(transformed_embedding, ((0, 0),(0, 0),(0, 0),(x0//16, x1//16)), mode='constant')
        
        output_image = resize(transformed_image, image.shape, order=self.image_interpolation, mode='constant', cval=0,
                                clip=True, preserve_range=True, anti_aliasing=True)
        output_label = resize(transformed_label, image.shape, order=self.label_interpolation, mode='constant', cval=0,
                                clip=True, preserve_range=True, anti_aliasing=False)
        output_feature = resize(transformed_feature, feature.shape, order=self.image_interpolation, mode='constant', cval=0,
                                clip=True, preserve_range=True, anti_aliasing=True)
        output_embedding = resize(transformed_embedding, embedding.shape, order=self.image_interpolation, mode='constant', cval=0,
                                clip=True, preserve_range=True, anti_aliasing=True)
        return output_image, output_label, output_feature, output_embedding
    ...
    def __call__(self, data, random_state=np.random):
        ...
        output = {}
        if 'feature' in data and 'embedding' in data:
            feature, embedding = data['feature'], data['embedding']
            image, label, feature, embedding = self.apply_rescale_neck(image, label, feature, embedding, sf_x, sf_y, random_state)
            output['image'], output['label'], output['feature'], output['embedding'] = image, label, feature, embedding
        else:
