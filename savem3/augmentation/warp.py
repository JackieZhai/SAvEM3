...
class Elastic(DataAugment):
    ...
    def __call__(self, data, random_state=np.random):
        ...
        for i in range(image.shape[-3]):
            if image.ndim == 3:
                transformed_image.append(cv2.remap(image[i], mapx, mapy, self.image_interpolation, borderMode=self.border_mode))     
            else:
        ...
        if 'feature' in data:
            transformed_feature = []
            for i in range(image.shape[-3]):
                temp = [cv2.remap(cv2.resize(data['feature'][channel, i], (width, height), self.image_interpolation), 
                        mapx, mapy, self.image_interpolation, borderMode=self.border_mode) for channel in range(data['feature'].shape[0])]
                transformed_feature.append(np.stack(temp, 0))
            transformed_feature = np.stack(transformed_feature, 1)
            output['feature'] = transformed_feature
        if 'embedding' in data:
            transformed_embedding = []
            for i in range(image.shape[-3]):
                temp = [cv2.remap(cv2.resize(data['embedding'][channel, i], (width, height), self.image_interpolation), 
                        mapx, mapy, self.image_interpolation, borderMode=self.border_mode) for channel in range(data['embedding'].shape[0])]
                transformed_embedding.append(np.stack(temp, 0))
            transformed_embedding = np.stack(transformed_embedding, 1)
            output['embedding'] = transformed_embedding
