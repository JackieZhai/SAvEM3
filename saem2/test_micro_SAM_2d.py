...
def get_args_parser():
    ...
    parser.add_argument("--output", type=str, 
                        default='work_dirs/test_micro_SAM_2d', 
                        help="Path to the directory where masks and checkpoints will be output")
    ...
    #                     help="The path to the SAM checkpoint to use for mask generation.")
    parser.add_argument("--checkpoint", type=str, 
                        default="/home/zhaih/share/SAM/sam-hq-main/train/pretrained_checkpoint_micro_sam/vit_h_em.pth", 
                        help="The path to the SAM checkpoint to use for mask generation.")
...
def main(train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        ...
        train(args, sam, optimizer, train_list_generator, valid_dataloaders, lr_scheduler)
    else:
        # sam = torch.nn.parallel.DistributedDataParallel(sam, device_ids=[args.gpu], find_unused_parameters=args.find_unused_params)
        ##### guojy
...
def train(args, sam, optimizer, train_list_generator, valid_dataloaders, lr_scheduler):
