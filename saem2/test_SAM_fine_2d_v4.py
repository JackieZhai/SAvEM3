...
def get_args_parser():
    ...
    parser.add_argument("--output", type=str, 
                        default='work_dirs/hq_sam_h_samfineisbi_2d_v4', 
                        help="Path to the directory where masks and checkpoints will be output")
    ...
    parser.add_argument("--restore-model", type=str, 
                        default='work_dirs/hq_sam_h_samfineisbi_2d_v4/epoch_0.pth',
                        help="The path to the hq_decoder training checkpoint for evaluation")
...
def main(train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        ...
        train(args, sam, optimizer, train_list_generator, valid_list_generator, lr_scheduler)
    else:
        # sam = torch.nn.parallel.DistributedDataParallel(sam, device_ids=[args.gpu], find_unused_parameters=args.find_unused_params)
        ##### guojy
...
def train(args, sam, optimizer, train_list_generator, valid_list_generator, lr_scheduler):
    ...
    for epoch in range(epoch_start,epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,1000):
            ...
            masks_hq = torch.cat(mask_outputs, 0)
