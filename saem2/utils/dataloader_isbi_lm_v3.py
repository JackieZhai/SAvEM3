...
def get_im_gt_name_dict(datasets, flag='valid', max_samples=100000):
    print("------------------------------", flag, "--------------------------------")
    name_im_gt_list = []
    ...
    for i in range(len(datasets)):
        ...
        for json_data_key, json_data_value in json_data.items():
            ...
            select_value_list = list(json_data_value.items())

            for neu_layer, useful_label_list in select_value_list:
                if neu_layer[-6:] == '_label':
                    neu_layer_name = neu_layer[:-6]
                else:
                    neu_layer_name = neu_layer

                for useful_label in useful_label_list:

                    single_dataset_part_list.append(None)
                    single_dataset_layer_list.append(neu_layer_name)
            ...
            random_num = min(len(single_dataset_part_list), max_samples)
            random.seed(42)
...
class OnlineDataset(Dataset):
    def __init__(self, name_im_gt_list, transform=None, eval_ori_resolution=False):
        ...
        self.datasets = {}
        self.embed_root_path = '/home/zhaih/share/SAM/sam-hq-main/train/prepared_embedding/'
        self.label_root_path = '/home/zhaih/share/SAM/sam-hq-main/train/prepared_segments_mul_2d/'
        self.mem_root_path = '/home/zhaih/share/SAM/sam-hq-main/train/prepared_segments_mul_mem/'
        dt_name_list = []
        da_part_list = []
        da_layer_list = []
        da_label_list = []
    ...
    def read_tensor_from_folder(self, folder_path, layer, label, flag='gt'):
        img_c = io.imread(os.path.join(folder_path, layer + '.tif'))
        if flag=='gt':
