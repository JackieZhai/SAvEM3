...
def masks_to_multi_boxes(masks):
    """Compute the bounding boxes around the provided masks
...
def masks_sample_same_points(masks, k=10):
    """Sample points on mask
    """
    if masks.numel() == 0:
        return torch.zeros((0, 2), device=masks.device)
    ...
    # k = 10
    select_mask = (masks>128)
    x_idx = torch.masked_select(x,select_mask)
    y_idx = torch.masked_select(y,select_mask)
    idx = torch.randint(0, x_idx.size(0), (k,), dtype=torch.int64, requires_grad=False)
    samples_x = x_idx[idx]
    samples_y = y_idx[idx]
    samples_xy = torch.cat((samples_x[:,None],samples_y[:,None]),dim=1)
    return samples_xy
