...
def get_args_parser():
    ...
    parser.add_argument("--output", type=str, 
                        default='work_dirs/hq_sam_h_memproisbi_2d_v4_iter', 
                        help="Path to the directory where masks and checkpoints will be output")
...
def train(args, net, optimizer, train_list_generator, valid_list_generator, lr_scheduler):
    ...
    for epoch in range(epoch_start,epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,1000):
            ...
            labels_points = misc.masks_sample_points(gt_c[:,0,:,:], k=1)
            ...
            for i in range(0, num_subiter):
                for dict_input, mask_hq, gt in zip(batched_input, masks_hq, gt_c):
                    mask_hq_inter = F.interpolate(mask_hq[None,:,:,:], gt.shape[-2:], mode="bilinear", align_corners=False)[0]
                    mask_diff = mask_hq_inter - gt/255.0
                    mask_fn = (-mask_diff) * (mask_diff < 0) * 255.0
                    mask_fp = mask_diff * (mask_diff > 0) * 255.0
                    
                    point_coords_add1 = misc.masks_sample_points(mask_fn[None,0,:,:], k=1)
                    if point_coords_add1.shape[1] == 0:
                        point_coords_add1 = misc.masks_sample_points(gt[None,0,:,:], k=1)
                    point_coords_add2 = misc.masks_sample_points(mask_fp[None,0,:,:], k=1)
                    if point_coords_add2.shape[1] == 0:
                        point_coords_add2 = misc.masks_sample_points(gt[None,0,:,:], k=1)
                    if 'point_coords' in dict_input:
                        dict_input['point_coords'] = torch.cat([dict_input['point_coords'],
                                                                point_coords_add1,
                                                                point_coords_add2], dim=1)
                        point_labels_add = torch.tensor([1, -1], device=dict_input['point_coords'].device)[None,:]
                        dict_input['point_labels'] = torch.cat([dict_input['point_labels'], point_labels_add], dim=1)
                    else:
                        dict_input['point_coords'] = torch.cat([point_coords_add1,
                                                                point_coords_add2], dim=1)
                        dict_input['point_labels'] = torch.tensor([1, -1], device=dict_input['point_coords'].device)[None,:]

                    if dict_input['point_coords'].shape[1] != dict_input['point_labels'].shape[1]:
                        pass
                ...
                loss_mask_add, loss_dice_add = loss_masks(masks_hq, gt_c/255.0, len(masks_hq))
                loss_mask_mem_add, loss_dice_mem_add = loss_masks(mask_mem, mem_c/255.0, len(masks_hq))
                loss_add = (loss_mask_add + loss_dice_add) + (loss_mask_mem_add + loss_dice_mem_add)
                
                loss_mask += loss_mask_add
                loss_dice += loss_dice_add
                loss_mask_mem += loss_mask_mem_add
                loss_dice_mem += loss_dice_mem_add
                loss += loss_add

            loss_mask = loss_mask / num_subiter
            loss_dice = loss_dice / num_subiter
            loss_mask_mem = loss_mask_mem / num_subiter
            loss_dice_mem = loss_dice_mem / num_subiter
...
if __name__ == "__main__":
    ...
    record_train = {"name": "record_train",
                    "path": './record2d_train.json'}

    train_datasets = [record_train,]
    valid_datasets = [record_train,] 
